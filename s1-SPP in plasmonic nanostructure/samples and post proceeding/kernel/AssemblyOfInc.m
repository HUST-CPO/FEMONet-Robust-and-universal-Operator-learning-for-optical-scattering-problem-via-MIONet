function solver=AssemblyOfInc(mesh,physic,solver)
...
for n=1:NbrSBC
    ...
    if numEdg==1
        xx(4)=xx(1);xx(5)=xx(2);
        yy(4)=yy(1);yy(5)=yy(2);
        l=sqrt((xx(1)-xx(2))*(xx(1)-xx(2))+(yy(1)-yy(2))*(yy(1)-yy(2)));
    elseif numEdg==2
        xx(4)=xx(1);xx(5)=xx(3);
        yy(4)=yy(1);yy(5)=yy(3);
        l=sqrt((xx(1)-xx(3))*(xx(1)-xx(3))+(yy(1)-yy(3))*(yy(1)-yy(3)));
    else
        xx(4)=xx(2);xx(5)=xx(3);
        yy(4)=yy(2);yy(5)=yy(3);
        l=sqrt((xx(3)-xx(2))*(xx(3)-xx(2))+(yy(3)-yy(2))*(yy(3)-yy(2)));
    end
    ...
    %submatrix
    PhySBC=1i*k0*refra;
    Ae=complex(0);be=complex(0);
    for k=1:IntegralEdgeOrder
        Ae=Ae+lp(k)*PhysicsFactor*PhySBC*dot(cross(normal,Et(:,k)),cross(normal,Et(:,k)));
    end
    for k=1:IntegralEdgeOrder
        be=be+2*lp(k)*PhysicsFactor*PhySBC*dot(cross(normal,Et(:,k)),cross(normal,[1,0,0]));
    end
    ...
    Av(NumNonA)=Ae;
    solver.b(mappingIndex)=solver.b(mappingIndex)+be;
end
