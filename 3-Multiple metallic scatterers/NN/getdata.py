...
class GetDataset(Dataset):
    def __init__(self, epsilon=None, coord=None, Ez=None, Ebz=None, Ai=None, Aj=None, Av=None, b=None, coord_len=None, index_offset=0):
        super().__init__()
        ...
        self.Ebz = torch.as_tensor(Ebz, dtype=torch.float64) 
    ...
    def __getitem__(self, index):
        ...
        epsilon = self.epsilon[index]  # (M,) or (2,H,W)
        ...
        return global_index, epsilon, coord, ez, ebz, ai, aj, av, b, coord_len
...
if __name__ == "__main__":
    # Load the data
    data_set = loadmat(DATA_DIR / "deepOnet_data_C_3456.mat")
    ...
    # Dataset instances (Ebz=None for DeepONet; MIONet needs Ebz_train, Ebz_test)
    Train_dataset = GetDataset(Epsilon_train, X_train, Ez_train, None, Ai_train, Aj_train, Av_train, b_train, coord_len_train)
    Test_dataset = GetDataset(Epsilon_test, X_test, Ez_test, None, Ai_test, Aj_test, Av_test, b_test, coord_len_test)

    # Example DataLoader (first item is global_index, includes ebz)
    loader = DataLoader(Train_dataset, batch_size=4, shuffle=True)
    for indices, epsilon_data, coord_data, E_true, ebz_data, Ai, Aj, Av, b, coord_len in loader:
        print(f'indices: {indices}, B_eps shape: {epsilon_data.shape}, T_xy shape: {coord_data.shape}, Ez shape: {E_true.shape}, ebz: {ebz_data.shape if ebz_data is not None else None}')
        break
