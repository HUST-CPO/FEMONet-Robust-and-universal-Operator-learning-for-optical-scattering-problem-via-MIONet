...
class GetDataset(Dataset):
    def __init__(self, epsilon=None, coord=None, Ez=None, Ebz=None, Lambda=None, Ai=None, Aj=None, Av=None, b=None, coord_len=None, index_offset=0):
        ...
        self.Lambda = torch.as_tensor(Lambda, dtype=torch.float64) if Lambda is not None else None  # (N, bi) MIONet third branch
    ...
    def __getitem__(self, index):
        ...
        ebz = self.Ebz[index]
        lambda_data = self.Lambda[index] if self.Lambda is not None else None  # (bi,)
...
if __name__ == "__main__":
    # Load the data
    data_set = loadmat(DATA_DIR / "deepOnet_data_B1_50688.mat")
    ...
    # Prepare dataset instances (MIONet 3-branch needs Ebz and Lambda)
    ...
    # Example DataLoader (returns global_index, epsilon, coord, ez, ebz, lambda_data, ai, ...)
