...
load(fullfile(mat_dir, 'Train_data_3Dcase3_285.mat'))
...
Eplison = reshape(eps_all,nbr,2,128,128,128);
% Ebz = reshape(Ebz_all,nbr,2,128,128,128);
...
testIdx = idx(1:testSize);  % first pctTrain fraction = test set
%testIdx = idx(testSize+1:end);  % remainder = test set
trainIdx = idx(testSize+1:end);  % remainder = train set
...
r_norm = (sum(abs(r)));
solver.A = A;
solver.b=b_train(i,1:coord_len_train(i)).';
mesh = Matrix{7,i};
P =  Matrix{8,i};
solver.x=P*Ez_train(i,1:coord_len_train(i)).';
figure('visible','on');
...
save(fullfile(mat_dir, 'deepOnet_data_3Dcase3_285.mat'), ...
     'X_train','X_test', ...
     'Eplison_train','Eplison_test', ...
     'Ez_train','Ez_test','Lambda_train','Lambda_test', ...
     'Ai_train','Ai_test', 'Aj_train','Aj_test','Av_train','Av_test', ...
     'b_train','b_test','coord_len_train','coord_len_test','-v7.3');      
save(fullfile(mat_dir, 'idx_3Dcase3_285.mat'),'trainIdx','testIdx');